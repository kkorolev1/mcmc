...
from mcmc.general_utils import to_var
from mcmc.total_variation import average_total_variation
...
def compute_metrics(
    ...
    try:
        ...
        metrics["tv_mean"] = tracker.mean().item()
        metrics["tv_conf_sigma"] = tracker.std_of_mean().item()
    except Exception as exc:
        print(f"Warning in TV calculation.\n{exc}")
        metrics["tv_mean"] = 1.0
        metrics["tv_conf_sigma"] = 0.0
